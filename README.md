...
The Rust CLI currently provides its initial executable scaffold, structured artifact loading and semantic validation, recursive OKF artifact discovery with profile diagnostics, Markdown AST validation and canonical ID extraction for Requirements, Design body-marker equality, typed canonical Contract parsing and project-wide graph validation, active Roadmap parsing and normalized review-scope fingerprinting, strict Cross-spec review candidate resolution, guarded accepted-state persistence, and accepted-review freshness evaluation, cross-artifact Requirement existence and active Design/Task coverage, canonical gate-input resolution and fingerprints, and cascading requirements/design/tasks freshness evaluation. Completion freshness, command rendering, remaining lifecycle transitions, and release behavior are being implemented incrementally from the accepted design decisions. Until that transition is complete, do not treat inherited cc-sdd behavior as the final SpecBind interface.
