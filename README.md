...
The Rust CLI currently provides its initial executable scaffold, the complete structural load pipeline for `spec.yaml` and `tasks.yaml`, and artifact-local semantic domain validation. It checks lifecycle state against gate evidence and active Requirement IDs, and checks task positions, local dependency graphs, cycles, and execution references. Project-context checks, commands, lifecycle transitions, and release behavior are being implemented incrementally from the accepted design decisions. Until that transition is complete, do not treat inherited cc-sdd behavior as the final SpecBind interface.
