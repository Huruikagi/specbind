...
The Rust CLI currently provides its initial executable scaffold, structured artifact loading and semantic validation, recursive OKF artifact discovery with profile diagnostics, canonical gate-input resolution and fingerprints, and cascading requirements/design/tasks freshness evaluation. Requirements, Design, and Contract body semantics, completion freshness, commands, lifecycle transitions, and release behavior are being implemented incrementally from the accepted design decisions. Until that transition is complete, do not treat inherited cc-sdd behavior as the final SpecBind interface.
