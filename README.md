...
The Rust CLI currently provides its initial executable scaffold and the complete structural load pipeline for `spec.yaml` and `tasks.yaml`: restricted YAML parsing, schema-version selection, embedded JSON Schema validation, and authoritative v1 wire-model deserialization. Commands, semantic lifecycle validation, and release behavior are being implemented incrementally from the accepted design decisions. Until that transition is complete, do not treat inherited cc-sdd behavior as the final SpecBind interface.
