...
The Rust CLI currently provides its initial executable scaffold, the complete structural load pipeline for `spec.yaml` and `tasks.yaml`, artifact-local semantic domain validation, canonical gate fingerprints, and cascading requirements/design/tasks freshness evaluation over discovery-resolved inputs. Project artifact discovery, completion freshness, commands, lifecycle transitions, and release behavior are being implemented incrementally from the accepted design decisions. Until that transition is complete, do not treat inherited cc-sdd behavior as the final SpecBind interface.
