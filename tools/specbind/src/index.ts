...
const helpText = `Usage: specbind [options]
...
const runPlanExecution = async (
  ...
  try {
    ...
    io.log('');
    io.log(formatBox(`specbind v${version} / ${agentDef.label}`));
...
export const runCli = async (
  ...
  if (parsedArgs.agent === 'codex') {
    ...
    io.log('');
    io.log(`  ${colors.bold('npx specbind@latest --codex-skills')}`);
    io.log('');
    return 1;
  }
