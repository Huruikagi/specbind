...
SpecBindはリリース名を解釈せず、大文字小文字も含めて文字列のまま扱います
（`v1.4.0`と`1.4.0`は別のリリースです）。スキルはこの値を自動で決めず、必ず尋ねます。指定するとMilestoneへ
...
ない状態でリリースした場合は、`sb-configure`でリリース後の設定レビューを行うことを
おすすめします。Steering、ルール、テンプレート、アダプターなど、更新が必要な対象を
そこで判断して担当スキルへ振り分けます。確定処理の後はSteeringの編集がふたたび自由になります。
