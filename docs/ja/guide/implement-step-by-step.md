...
番号の位置を残し、対象の受け入れ基準の先頭に`_Retired_`を付けます。
...
配下すべてを退役扱いにします。元の文章は必要に応じて残せます。マーカーだけの
項目や見出しも有効です。退役した位置やグループ番号は削除・再利用しません。
義務の継承・終了は、対象の直下に通常の文章や下位リストで説明します。

今回退役するbaselineのIDは、すべて作業対象として承認します。DesignとTasksで
退役への対応を計画し、最終検証で結果を確認します。マーカー自体は完了の証拠では
ありません。過去に退役したIDを再び選ぶことはできません。Specのすべての義務を
退役させる操作には、まだ対応していません。

既存RequirementsのID移行は不要です。利用前にインストール済みSkillと、変更している
場合はテンプレートの指示を更新してください。旧バイナリはマーカーを正しく解釈できないため、
利用開始後は旧バージョンに戻さないでください。
