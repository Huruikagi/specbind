...
既存のRequirementをなくす場合は削除せず、[要件を退役させる](#retire-requirements)の
方法で退役させます。
...
検証方法は、何を証明するかに加え、必要な入力や状態をTask完了時点でどう用意できるかも
確認します。ほかのSpecが提供する機能を使う場合は、その責務とRoadmapの実装順を
照合します。既存の入口へのテスト入力で自分の責務を検証できるなら、実際の接続を通す
統合検証は接続後に配置できます。Specを単独で完了させるためだけに新しい公開APIや
抽象化を増やす必要はありません。完了条件や検証の配置、作業順を見直し、承認済みの
Designなどに変更が必要なら、その所有フェーズへ戻します。
...
各Taskでは、その変更に適用される必須チェックを実行します。追加テストは、変更した
振る舞いや回帰を確認できる場合に作成し、文章だけの修正に同じ文言を照合するテストを
増やす必要はありません。必須チェックが通ったらレビューへ進みます。再検証や検証範囲の
拡大は、その後の変更、失敗、未解決の懸念、プロジェクトの明示的な規定がある場合に行います。
次の手順で行うSpec全体の完了検証は省略しません。
...
完了状態を更新せず「本当に終わったか」だけを確認する場合は、`sb-verify-completion`を
使います。`sb-status`は現在地の説明、`sb-validate-implementation`は完了記録を受理する
ための検証です。実行済みチェックの結果を報告するたびに、別の検証スキルを呼ぶ必要はありません。
...
## 補足: 要件を退役させる {#retire-requirements}

確立済みのRequirementをなくすときは、削除せずに退役させます。番号の位置を残し、
対象の受け入れ基準の先頭に`_Retired_`を付けます。

```markdown
2. _Retired_ カートの商品数を10個までに制限する。
   - 件数制限を撤廃する。在庫確認は要件3.4に残す。
```

グループ全体なら、見出しを`### 要件 3: _Retired_ カートの集計`のようにして、
配下すべてを退役扱いにします。

- 元の文章は必要に応じて残せます。マーカーだけの項目や見出しも有効です。
- 退役した位置やグループ番号は削除・再利用しません。
- 義務の引き継ぎや終了は、対象の直下に通常の文章や下位リストで説明します。

今回退役する既存のIDは、すべてRequirements Gateで作業対象として承認されます。
DesignとTasksで退役への対応を計画し、最終検証で結果を確認します。マーカー自体は
完了の証拠ではありません。過去に退役したIDを再び選ぶことはできません。Specの
すべての義務を退役させる操作には、まだ対応していません。
