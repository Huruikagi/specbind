...
成功結果の`source_revision`は調査対象を固定するGit commitです。調査中に
...
調査結果は一時的な
`{{SPEC_DIR}}/adoption/reverse-discovery.yaml`に記録されます。各Observationは、
固定されたrevision上のパスと、symbol、テスト名、route、schema entryなどの
位置を持ちます。

Observationは次のいずれかへ振り分けます。
...
すべてのSpecについてBriefとResearchへの引き継ぎが完了すると、プロジェクト単位の
dossierは現在のtreeから削除されます。Git履歴には調査経緯が残り、Specごとの
Researchは通常のrelease finalizationまで保持されます。
