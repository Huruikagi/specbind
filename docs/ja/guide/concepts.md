...
仕様駆動で開発をしていると、継続的に開発をして規模が大きくなるにつれ
トークン消費量が線形以上に増えていくと感じています。

仕様駆動開発では、実装だけでなくドキュメントも同時にメンテナンスしていく以上、
ある程度避けられない課題と考えています。

この課題に向き合っていくために、SpecBindではいくつかの実験的なアプローチを適用しました。

## SkillとCLI

仕様と設計と実装とを実際に対応させていくのはAIエージェントの仕事です。
エージェントは優秀なので、機械検査のためのルールをスキルとして与えれば
ワンライナーのシェルを都度構築してくれますが、
この定型作業のために毎回トークンを消費するのも勿体ないかもと考えました。

そこで、定型的な機械検査などを専門で行うCLIを提供することで、
エージェントは必要な時にCLIの決まったコマンドを呼び出す形を採用しています。

SpecBindでは、責任を次のように分けています。

| 担当 | 主な責任 |
| --- | --- |
| エージェントのSkill | スコープ判断、要求や設計資料の作成、レビュー、実装、結果の説明 |
| `specbind` CLI | 構造検証、トレーサビリティ、承認の証拠、進捗、状態遷移、リリース前チェック |
| あなた | スコープの確定、必要な承認、プロジェクト固有の判断、公開結果の確認 |

SkillはCLIが管理している状態（具体的には `spec.yaml` ファイル）を直接書き換えません。
逆にCLIは、要求の意味が正しいか、設計が妥当かといった判断をしません。

両方の層を必ず通すことで、「形式だけ整った仕様」や、
「内容はもっともらしいのに状態遷移を飛ばした作業」を防ぐこともできます。

## Spec

Specは、プロジェクトが持ち続ける1つの能力、あるいは責任の境界です。

SpecBindで開発を進めるとプロジェクトは1つ以上のSpecを持っていくことになります。

ここで、Specは変更のたびに使い捨てる計画書（チケット）ではありません。
あとのMilestoneで同じ機能に修正を加えるときは、
既存のSpecのRequirements、Design、Contractを「現在の姿」として更新します。
リリース後もSpecは残り、次の変更の出発点になります。
...
Milestoneは、1回のリリースとしてまとめて届ける作業の単位です。

- 作成されたり更新されるSpec（Spec-backed item）
- Specとは関係ない直接実装（Direct item）
- 項目どうしの依存関係
- 対象リリース

などをRoadmapとして記録します。

アクティブなMilestoneは、プロジェクトごとに1つだけです。
現在のRoadmapは`.specbind/steering/roadmap.md`にあり、状態はCLIが管理します。
リリースが完了すると、Roadmapの内容はアーカイブされます。
...
変更要求のプロンプトを投げると、たいてい specbind-discovery（Discovery）
Skillが呼び出されるようになっています。もちろん明示指定してもかまいません。
Discoveryはあなたが依頼した変更をSpecBindとしてどう扱うかを振り分ける役割をします。

まず、Discoveryは、ワークフローに入ってきた作業を「誰が所有するか」で分類します。
...
`.agents/skills/`はCodexと`generic` Agentで共有されます。`generic`は、共通形式の
Skillと`AGENTS.md`だけを導入し、製品固有のサブエージェント定義は作りません。
