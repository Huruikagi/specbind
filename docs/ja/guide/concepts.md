...
トークン消費量がやたら増えていくと感じています。

実装だけでなくドキュメントも同時にメンテナンスしていく以上、
ある程度避けられない課題でしょう。
...
エージェントは優秀なので、機械検査のためのルールをSkillとして与えれば
ワンライナーのシェルを都度構築してくれるでしょう。
ただ、この定型作業のために毎回トークンを消費するのも勿体ないかもと考えました。
...
SpecBindで開発を進めていくとプロジェクトは1つ以上のSpecを持っていくことになります。
