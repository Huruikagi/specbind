...
**`sb-drive`** — Milestone全体から安全に到達可能な所有ワークフローを通常は1つずつ選び、
各委譲後にCLI状態を読み直します。局所的な判断待ちは保留して独立項目を続けますが、
リリースは実行せず、その手前で停止します。`--parallel [<limit>]`を指定すると、対応が確認
できた環境では独立したSpecを別ワークツリーで並行実装し、1件ずつ統合します。各Spec内の
Taskは直列のままです。必要な機能がない環境では、従来どおり直列に進めます。
