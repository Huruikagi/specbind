# Why SpecBind? A philosophy note

> English | [日本語](ja/why-specbind.md)

This is the long version of "why does SpecBind exist and what trade-off is it making". If you just want to install and try it, the project [README](../../README.md) is the faster path. Come here when you want to understand the design rationale.

## The short version

SpecBind treats the spec as a contract between parts of the system, not a master command document to be handed to the agent. Code remains the source of truth. Specs make the contracts between parts of the code explicit, so humans and agents can work in parallel without constant synchronization.
...
specbind treats this distinction as first-class:
...
## How specbind thinks about speed

The usual assumption about specs is that they slow you down. specbind is built on the opposite assumption: explicit contracts at the right granularity make AI-driven development at team scale faster, not slower. Four things follow from that stance.
...
## When specbind is the right tool
...
Even inside specbind, `/kiro-discovery` can legitimately return *"no spec needed, implement directly"* as a valid route. specbind is not trying to put a formal spec around every change, only the ones where the contract between parts of the system actually earns its cost.

> If the discipline feels like overhead, your specs are probably too big. Break them smaller.

## See also

- [Spec-Driven Development Workflow](spec-driven.md): how the ideas here are implemented as an end-to-end workflow in specbind.
- [Skill Reference](skill-reference.md): the skills-mode surface, including `/kiro-impl` dispatch internals.
- [Migration Guide](migration-guide.md): if you are coming from specbind v1.x or v2.x.
