...
| `sb-drive` | Drive safe reachable work and stop before Release; optional `--parallel [<limit>]` runs one independent Spec implementation batch in verified worktrees and stops with retained branches for separate integration; unsupported hosts fall back to sequential execution; optional `--replan` delegates Design, Contract, and Tasks recovery within approved Requirements and Milestone scope, then resumes delivery. |
