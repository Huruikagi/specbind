...
| `sb-drive` | Drive safe reachable work and stop before Release; optional `--parallel <limit>` batches independent Spec implementation in verified worktrees and falls back to sequential execution on unsupported hosts; optional `--replan` delegates Design, Contract, and Tasks recovery within approved Requirements and Milestone scope, then resumes delivery. |
