...
| --- | --- | --- | --- | --- | --- |
| FT-0019 | `086620a` | CF3 | A request to configure future API and infrastructure Design templates can be read as authority to add candidates even when no current Steering or repository fact establishes those independent responsibilities. | Pending. | Rerun CF3 after the Skill asks the maintainer to choose between existing-surface and conditional-candidate options. |
