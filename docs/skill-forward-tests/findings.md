...
| --- | --- | --- | --- | --- | --- |
| FT-0025 | `51f765c` | U1 | A natural request to update SpecBind can bypass `sb-configure` and treat project-asset refresh as the complete update when project instructions do not route the explicit update before ordinary change classification. | The managed project-instruction block now routes explicit binary, mise-selector, and update-coupled asset requests directly to `sb-configure`. | Rerun fresh U1 and confirm ownership is checked before any install apply. |
| FT-0026 | `51f765c` | U2 | The update reference requires binary selection and asset refresh as two checkpoints, while the general Git adapter described every `sb-configure` change and aftercare as one checkpoint. | The English and Japanese default Git adapters now state the explicit update workflow's two-checkpoint exception and clean-worktree reason. | Rerun fresh U2 and confirm two checkpoints without interpretation of precedence. |
