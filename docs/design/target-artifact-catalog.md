...
| `{{SPEC_DIR}}/adoption/reverse-discovery.yaml` | Exists only while reverse establishment is deriving Specs from a committed implementation baseline. | Authored and retired by reverse mode in `sb-discovery`. | Accepted | A transient investigation ledger under Decision 0181, not lifecycle state or a strict wire model. It records the source revision, selected scope, proposal, evidence-backed observations, and dispositions until adoption finalization. |
...
| `{{SPEC_DIR}}/baselines/<version>-roadmap.md` | Persists reverse scope and fixed-revision provenance without claiming a product release. | `sb-discovery` through guarded Rust finalization. | Accepted | Decision 0181 adoption history; version is the existing `baseline_version`. |
| `{{SPEC_DIR}}/baselines/<version>-contract-review.md` | Persists the accepted Contract Review for a reverse-established baseline. | `sb-discovery` through guarded Rust finalization. | Accepted | Companion adoption archive; it is not release evidence. |
