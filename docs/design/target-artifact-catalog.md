...
| --- | --- | --- | --- | --- |
| `{{SPEC_DIR}}/index.md` | Installed with the Knowledge Bundle and retained across milestones and releases. | SpecBind maintains the localized marked navigation block and OKF version declaration; the project owns all other body text. | Accepted | Decision 0212 makes this the static bundle entry point, not a generated inventory or lifecycle authority. Installation creates a missing file, additively adopts an existing compatible index, and later replaces only the exact managed block. |
...
| `{{SPEC_DIR}}/adoption/reverse-discovery.yaml` | Exists only while reverse establishment is deriving Specs from a committed implementation baseline. | Authored by `sb-adopt` and retired through guarded reverse finalization. | Accepted | A transient investigation ledger under Decisions 0181 and 0196, not lifecycle state or a strict wire model. It records the source revision, selected scope, proposal, evidence-backed observations, and dispositions until adoption finalization. |
...
| `{{SPEC_DIR}}/baselines/<version>-roadmap.md` | Persists reverse scope and fixed-revision provenance without claiming a product release. | `sb-adopt` through guarded Rust finalization. | Accepted | Decision 0181 adoption history; version is the existing `baseline_version`. |
| `{{SPEC_DIR}}/baselines/<version>-contract-review.md` | Persists the accepted Contract Review for a reverse-established baseline. | `sb-adopt` through guarded Rust finalization. | Accepted | Companion adoption archive; it is not release evidence. |
...
| `{{SPEC_DIR}}/settings/adapters/validation.md` | Installed as a scaffold and maintained as optional project configuration. | Project maintainers; proposed by `sb-configure` and consumed by `sb-validate-implementation`. | Accepted | Free-form OKF project guidance under Decision 0182. Active applicable procedures add project-specific final-validation evidence requirements; absence, a scaffold, or an empty body adds none. It cannot weaken canonical checks or product protocols, grant authority, or let the validator repair its own findings. |
