...
Under [Decisions 0011](./decisions/0011-cross-spec-contract.md), [0055](./decisions/0055-cross-spec-review-inputs.md), [0155](./decisions/0155-versioned-yaml-contract-artifact.md), and [0078](./decisions/0078-contract-first-review-between-design-and-tasks.md), the CLI loads every current persistent fixed `contract.yaml`, validates its versioned structure and the complete dependency graph, and fingerprints the normalized typed input set. It reports duplicate IDs, unresolved references, missing manifests, and dangling references as errors. Ownership overlaps and dependency cycles are review warnings rather than unconditional structural failures.
...
[Decision 0156](./decisions/0156-derived-contract-graph-reads.md) adds the
non-persisted inspection projections over the same validated graph:
...
current Contract set respectively, and their success output follows the
Decision 0067 contract:
...
The CLI owns template discovery, identity and path validation, collision checks, instruction-node recognition, non-writing output-tree previews, and the read-only `configuration show` projection. `specbind-configure` owns navigation and completion across the supported configuration surfaces while delegating semantic authoring to the owning workflow when needed. Ordinary artifact workflows own materialization through their guarded lifecycle operation; template read commands never create or overwrite live artifacts.
