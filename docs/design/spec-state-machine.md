...
Status: Implemented in v1; the open questions below remain.

This document defines the target per-spec workflow states and events stored through `spec.yaml` under [Decision 0014](./decisions/0014-structured-spec-metadata.md). It refines the active requirement set accepted in [Decision 0003](./decisions/0003-active-requirement-set.md), and the approval modes accepted in [Decision 0012](./decisions/0012-delegated-approval.md).
...
| `adoption_ready` | `active_change.state: "adoption_ready"` | A reverse-established Spec has current Requirements and Design approval and awaits milestone Contract Review or adoption finalization. | Reverse establishment provenance plus valid requirements and design gate evidence; no `tasks.yaml`. |
...
active_change:
  ...
  gate_evidence:
    requirements:
      ...
      delegation_workflow: specbind-plan
...
Gate evidence must identify the approved input revision strongly enough for the CLI to detect a later out-of-band edit. Fingerprint values use the `sha256:<64 lowercase hex characters>` representation accepted in [Decision 0016](./decisions/0016-fingerprint-value-format.md). Under [Decision 0057](./decisions/0057-type-based-artifact-discovery.md), the CLI discovers spec Markdown by OKF type and evidence binds logical artifact identities rather than filenames; Decision 0155 gives the Contract the fixed `contract.yaml` locator and a normalized typed fingerprint. The requirements gate covers the singleton requirements artifact at `input_revisions["requirements"]` and the ordered active Requirement ID set but excludes the brief under Decision 0017. Markdown line endings are normalized before hashing, while the ordered Requirement IDs are stored directly as `approved_requirement_ids` and compared by exact array equality under Decision 0018. The design gate fingerprints the required Contract at `contract` and the complete current design set at `design/<artifact_id>` under Decision 0038. The tasks gate stores only the normalized typed `plan` projection fingerprint at `input_revisions["tasks.yaml#plan"]` under Decisions 0028 and 0039. Every gate uses a timezone-qualified RFC 3339 `passed_at` under Decision 0036 for the time that the current revision passed; gate transition and evidence persistence are one guarded mutation, so the target schema has no separate `approved_at` or `recorded_at`. `explicit` records approval after the current revision exists and omits `delegation_workflow`. `delegated` requires `delegation_workflow` to identify the accelerated or batch workflow that crossed the gate. The delegation itself remains only in that workflow's run context and is not persisted in `spec.yaml` or another project artifact. Both modes satisfy the same gate guards; only the post-gate confirmation pause differs. `--non-interactive` controls prompting and never creates approval authority.
...
| `DESIGN_CHANGED` | Declare that approved design or contract input changed and invalidate design and downstream gates. | Design or contract review workflow through the Rust CLI contract. |
...
| `IMPLEMENTATION_VALIDATED` | Accept fresh spec-level completion while requiring the project-state-owned global contract-impact and downstream-review record to be current for the complete milestone. | Integration validation / completion workflow through the Rust CLI contract. |
...
| --- | --- | --- | --- | --- |
| `SPEC_CREATED` | Spec does not exist | `requirements` | An active roadmap exists; the new spec boundary and canonical identity are confirmed in scope; no conflicting spec path exists; the milestone ID is valid. Authoring the active brief and initial requirements scaffold is the responsible workflow's obligation after the guarded mutation, not a CLI precondition; see [Decision 0089](./decisions/0089-milestone-creation-cli.md). | Create the persistent spec artifacts and `active_change`; set `requirement_ids: null`; create no approval evidence. |
| `CHANGE_STARTED` | `idle` | `requirements` | An active roadmap exists; the canonical spec identity is confirmed in scope; no active change exists; the milestone ID matches roadmap membership. The active brief is authored by the responsible workflow after the guarded mutation. | Create `active_change`; set `requirement_ids: null`; create no approval evidence. |
...
| `IMPLEMENTATION_VALIDATED` | `implementation` | `release_ready` | Required Tasks are complete and unblocked; the mandatory semantic validation protocol produces `GO`; the global review is fresh for the complete Spec-backed milestone; the unchanged clean Git revision satisfies the Decision 0029, 0080, and 0086 handshake. | Record Spec-local completion evidence and transition. Semantic pass flags and contract review data are not copied into `spec.yaml`. |
| `COMPLETION_INVALIDATED` | `release_ready` | `implementation` | Requirements, design, contract, and tasks remain approved; the validated implementation revision or another completion input changed; otherwise use the corresponding earlier invalidation event. | Through the Decision 0086 explicit command, clear completion evidence only. |
