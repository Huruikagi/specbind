...
| `IMPLEMENTATION_VALIDATED` | `implementation` | `release_ready` | Required tasks are complete and unblocked; the mandatory semantic validation protocol produces `GO`; contract-impact and downstream-review evidence pass; the unchanged clean Git revision and lifecycle inputs satisfy the Decision 0029 handshake. | Atomically record completion evidence and its input revisions, then transition. Semantic pass flags are not persisted under Decision 0034. |
