...
| Integration validation | Cross-task behavior, full verification, spec coverage, and run-scoped candidate evidence produced between the CLI completion preflight and guarded acceptance calls | Replacing missing task-level review, directly mutating completion state, or persisting failed attempts in lifecycle metadata |
