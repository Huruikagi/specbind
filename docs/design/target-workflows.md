...
| Release agent orchestration | Read the adapter, call stateless `specbind release preflight`, execute project phases, prepare per-spec summaries, call CLI finalization, and report outcomes | Treating preflight as mutation authority, reimplementing lifecycle mutations, or bypassing core gates |
