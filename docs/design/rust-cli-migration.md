...
The target structured artifacts remain `spec.yaml` and `tasks.yaml` under Decisions 0013 and 0014. The preparatory directory move temporarily carries the current hand-authored schema copies under `tools/cc-sdd/schemas/`; after the Rust toolchain and workspace bootstrap, the first schema implementation increment introduces the authoritative versioned wire models accepted by Decision 0085, generates their distribution schemas at `tools/specbind/schemas/`, and embeds them into the matching binary. Wire models, generated schemas, and domain conversion share conformance fixtures. Shared `settings/templates/` and `settings/rules/` are user-owned customization surfaces; generated agent assets are product-managed.
