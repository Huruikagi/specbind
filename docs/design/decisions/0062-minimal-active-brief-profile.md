...
- Release-log authoring may use the brief as drafting context, but it must derive the delivered summary from final requirements, active Requirement IDs, completed tasks, roadmap scope, accepted completion evidence, and contract review. The brief is not authoritative released state.
...
- Released history remains concise in `log.md`, while ordinary Git history normally retains the complete final working brief.
