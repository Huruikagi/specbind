...
- `accept` is the sole `IMPLEMENTATION_VALIDATED` mutation. It independently recomputes every preflight guard, requires the submitted full revision to equal clean current `HEAD`, atomically records completion evidence, and transitions the Spec from `implementation` to `release_ready`.
...
- The existing cross-spec review acceptance and lifecycle gate operations must be exposed through stable CLI commands before the complete generated validation workflow can rely only on public Rust CLI surfaces.
