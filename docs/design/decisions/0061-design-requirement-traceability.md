...
The Rust Design parser recognizes only complete plain-text Markdown emphasis nodes matching the exact `Requirements: N.M, ...` grammar, unions repeated markers, and validates bidirectional set equality with the artifact's structurally valid Front Matter `requirement_ids`. Bare text, strong emphasis, code, comments, partial emphasis, and nested inline constructs do not create references. Discovery reports missing markers and both mismatch directions with complete document line numbers. The cross-artifact traceability resolver now checks every Design mapping against the current Requirements catalog and requires the Design collection union to cover every known active Requirement ID from a structurally and semantically valid `spec.yaml`; inactive current mappings remain allowed. Task coverage and CLI rendering remain subsequent increments.
