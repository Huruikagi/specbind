...
Not implemented. The persisted evidence wire models, gate-input resolution, fingerprints, freshness chain, semantic state invariants, and the Tasks-approval cross-spec review boundary guard already exist. Clap routing, the guarded approval and invalidation mutations, concise rendering, stable exit behavior, and CLI integration tests for these six commands remain to be implemented.
