...
Six of the eight accepted protocol documents are authored and registered: `okf-authoring`, `requirements-review`, `design-discovery`, `design-authoring`, `design-validation`, and `gap-analysis`. The remaining two selectors are not yet registered, so `protocol list` currently reports six entries. Authoring them, referencing them from cross-agent skills, and adding the version-mismatch and customization-cannot-override tests remain separate increments.
