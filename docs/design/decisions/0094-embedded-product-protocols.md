...
Four of the eight accepted protocol documents are authored and registered: `okf-authoring`, `requirements-review`, `design-discovery`, and `design-authoring`. The remaining four selectors are not yet registered, so `protocol list` currently reports four entries. Authoring them, referencing them from cross-agent skills, and adding the version-mismatch and customization-cannot-override tests remain separate increments.
