...
All eight accepted protocol documents are authored and registered, so `protocol list` reports the complete set. Referencing them from cross-agent skills and adding the version-mismatch and customization-cannot-override tests remain separate increments that depend on the v1 skill assets.
