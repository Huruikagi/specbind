...
Three of the eight accepted protocol documents are authored and registered: `okf-authoring`, `requirements-review`, and `design-discovery`. The remaining five selectors are not yet registered, so `protocol list` currently reports three entries. Authoring them, referencing them from cross-agent skills, and adding the version-mismatch and customization-cannot-override tests remain separate increments.
