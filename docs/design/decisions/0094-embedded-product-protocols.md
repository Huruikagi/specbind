...
Two of the eight accepted protocol documents are authored and registered: `okf-authoring` and `requirements-review`. The remaining six selectors are not yet registered, so `protocol list` currently reports two entries. Authoring them, referencing them from cross-agent skills, and adding the version-mismatch and customization-cannot-override tests remain separate increments.
