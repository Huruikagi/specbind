...
- `Consumes` uses `- \`<local-id>\` → \`<canonical-spec>/<target-section>/<target-id>\`` and may append ` — <description>`. The target section is one of `owns`, `exports`, `invariants`, or `file-ownership`; `consumes` entries are never dependency targets.
...
- File Ownership paths are project-root-relative POSIX paths. Absolute paths and `.` or `..` segments are invalid. V1 accepts only an exact path or a terminal subtree form ending in `/**`; it does not implement general glob syntax. Comparison is ASCII case-insensitive so a contract has one portable interpretation on Windows and Linux.
