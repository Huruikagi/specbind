...
- Abandoning an entire unreleased milestone is an explicit, user-confirmed lifecycle operation. It may close active-change metadata and remove each milestone-local discovered `SpecBind Brief` and optional `SpecBind Research` artifact, fixed `tasks.yaml`, `steering/roadmap.md`, and the matching `state/cross-spec-review.md` only after affected repository and active-spec content has been restored or reconciled.
- An abandoned unreleased milestone is not added to per-spec `log.md` and its roadmap is not archived under `releases/` by default. Git history remains available when the abandoned work was committed.
...
- Keep released rollback distinct from unreleased abandonment so the per-spec release log remains an index of actual releases.
