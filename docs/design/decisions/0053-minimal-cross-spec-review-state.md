...
- `input_revisions` is a non-empty mapping whose values use the Decision 0016 fingerprint representation. It identifies the source artifact revisions from which classifications and the final judgment were produced. The exact owned projections, path-key rules, and normalization remain a follow-up decision.
