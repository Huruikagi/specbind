...
- Successful release finalization moves the roadmap to `{{SPEC_DIR}}/releases/<version>/roadmap.md`.
...
- Require the active roadmap's bound release version to match the archive directory.
...
- `releases/` becomes the durable project-level release-history area.
...
- Whether other project-level release summaries or evidence files will join the archived roadmap directory.
- How cancelled, never-released milestones are archived and named.
- Whether release version directory names use a normalized leading `v` in every project.
