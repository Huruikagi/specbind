...
- A spec may contain `implementation-notes.md` alongside its persistent requirements, design, and contract artifacts.
- The file is optional and is created only when useful implementation knowledge needs to be retained.
- `implementation-notes.md` is free-form Markdown. SpecBind defines no required headings, entry schema, Task ID links, authors, timestamps, categories, or stable note IDs.
...
- Successful release finalization preserves `implementation-notes.md` unchanged while removing the milestone-local `tasks.yaml`.
...
- Non-empty content under an inherited `tasks.md` `## Implementation Notes` heading moves to the spec's `implementation-notes.md`, preserving its useful Markdown structure and prose.
