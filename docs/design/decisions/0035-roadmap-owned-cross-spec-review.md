...
The active `roadmap.md` already owns milestone scope, dependency ordering, and cross-spec evidence. Each active spec already identifies its roadmap scope through `active_change.milestone_id` and `active_change.change_id`.
...
- A cross-spec review record is associated with the relevant milestone item or per-spec change identity. One shared record may cover several affected specs; it is not copied into every `spec.yaml`.
...
- The CLI resolves the relevant roadmap record through the active change's existing `milestone_id` and `change_id`. No additional roadmap-evidence reference field is added to `spec.yaml` v1.
