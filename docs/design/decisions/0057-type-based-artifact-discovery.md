...
The Rust discovery boundary recursively scans regular lowercase `.md` files without following symbolic links, validates recognized singleton and collection metadata profiles, assigns logical selectors, removes ambiguous duplicates from the usable partial inventory, and orders results by the accepted inventory sequence. It also diagnoses empty Research and Implementation Notes bodies, leaked live `specbind:instruction` comments, Decision 0060 Requirements body violations, per-Design Decision 0061 marker equality, and Decision 0056 canonical Contract body violations. The traceability resolver builds on this inventory and the validated structured artifacts to check Design and Task references plus active-scope coverage. The project-wide Contract resolver retains each partial inventory while requiring one valid singleton Contract per persistent Spec and building the complete typed dependency graph.
