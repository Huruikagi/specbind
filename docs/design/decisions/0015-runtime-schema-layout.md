...
The Rust workspace owns the complete structural load pipeline and v1 wire models for the accepted `spec.yaml` and `tasks.yaml` fields. Its restricted parser rejects duplicate keys, anchors, aliases, merge keys, custom tags, multiple documents, and values that cannot be represented as neutral JSON. The runtime loader then selects `schema_version`, evaluates the matching embedded artifact schema with format assertions enabled, and deserializes schema-valid values into the matching wire model. Parser, version-selection, schema, and wire failures remain distinct internal error layers. A separate domain conversion reports all artifact-local semantic contradictions with stable codes while preserving the structurally readable wire value for status and repair workflows. Rust-owned fixtures exercise each implemented layer; the inherited TypeScript artifact generation and validation remain unchanged migration inputs.
