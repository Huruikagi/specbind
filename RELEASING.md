...
5. Run the non-publishing native release preflight with the exact Cargo
   version and wait for all four archives and checksums to pass:
...
The release workflow verifies all four native targets, creates the archives and
...
Verify the checksum file and all four archive entries on the GitHub Release page.
Then test explicit installation on the documented native environments.

The release workflow uses a pinned mise version for its four-target smoke
...
selects the `aarch64-apple-darwin` archive. Linux ARM64 selects
...
and language. Record the exact Windows, WSL2/Linux x64, Linux ARM64, and macOS ARM64 versions
